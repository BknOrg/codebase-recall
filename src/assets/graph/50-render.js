...
function draw() {
  ...
  for (const l of links) {
    ...
    const isOutbound = hovered && s.id === hovered;
    const isInbound = hovered && t.id === hovered;

    const near = isOutbound || isInbound;
    const on = hs ? hs.has(s.id) && hs.has(t.id) : true;

    if (l.kind === "contains") {
      ctx.globalAlpha = near ? 0.5 : on ? 0.16 : 0.05;
      ctx.strokeStyle = isOutbound ? COL.accent : isInbound ? "#0ea5e9" : COL.edge;
      ctx.lineWidth = 0.8 / k;
      ctx.beginPath();
      ctx.moveTo(s.x, s.y);
      ctx.lineTo(t.x, t.y);
    ...
    const lineColor = isOutbound ? COL.accent : isInbound ? "#0ea5e9" : COL.edge;
    ...
    ctx.lineTo(t.x, t.y);
    ctx.stroke();
    if (k > 0.4) drawArrow(s, t, near, on, lineColor);
  }
  ctx.globalAlpha = 1;
...
function drawArrow(s, t, near, on, color) {
  const dx = t.x - s.x;
  const dy = t.y - s.y;
  ...
  ctx.fillStyle = color || (near ? COL.accent : COL.edge);
  ctx.beginPath();
  ctx.moveTo(bx, by);
